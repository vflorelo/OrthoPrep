...
do
    ...
    then       
        ...
        parallel -j ${threads} run_flps ::: "${seq_id_list[@]}" ::: Species${num}.seq.tsv ::: Species${num}.fa.COMPOSITION | sort -V | uniq > Species${num}.flps.tsv 2> Species${num}.flps.err
        ...
        parallel -j ${threads} get_eff_len ::: "${seq_id_list[@]}" ::: "${sizes_tsv_file}" ::: "Species${num}.lcr.bed" | sort -V | uniq > "Species${num}.lcr.tsv"
        if [ "${no_masking}" == "FALSE" ]
        then
